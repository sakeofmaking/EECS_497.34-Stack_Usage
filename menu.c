...
{ 
	 ...
	 * would be:
	 * 1. Starting at the end of RAM (0x4FF), check for the pattern 0xAA.
	 ...
	 *    b) You get to the start of RAM at 0x100. Clearly, all RAM has been
	 *       used at some point, and somehow, you didn't crash.
	 */

	int StackCount = 0;
	unsigned int address = MAX_MEM_ADDR;
	unsigned char *addr_ptr = (unsigned char *)address;
	int pattern_count = 0;

	while(pattern_count != RAM_MIN_PATTERN){
		if(*addr_ptr == 0xAA){
			pattern_count++;
		} else if(addr_ptr == (unsigned char *)0x200){
			break;
		}
		StackCount++;
		addr_ptr--;
